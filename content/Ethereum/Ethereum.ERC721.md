...
ERC721 is the standard for non-fungible tokens on Ethereum.
...
Notice that `ERC721` does not contains a method `transfer` like `ERC20`, the functionality is included by `transferFrom`.

`transferFrom` would throw when:
...
and `safeTransferFrom` would not only throw with the former situations, but also:
